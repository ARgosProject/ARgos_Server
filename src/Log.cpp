...
namespace argosServer {
  ...
  void Log::paper(const Paper& paper, const std::string& filename) {
    if(coloured_output)
      std::cout << "\033[" << Colour::FG_DARK_GRAY << "m";

    std::cout << currentDateTime() << " [PAPER] " << paper << std::endl;
    ...
    if(!filename.empty()) {
      std::ofstream ofs(filename, std::ofstream::app);
      ofs << currentDateTime() << " [PAPER] " << paper << std::endl;
      ofs.close();
    }
  }
