...
void Paper::glGetModelViewMatrix(float modelview_matrix[16]) throw(cv::Exception){
  ...
  if (scale != 0.0){
	...
	/*
	// Inverse YZ rotation
	cv::Mat auxRotVec = rotVec.clone();
	auxRotVec.at<float>(1,0) *= -1.0f;
	auxRotVec.at<float>(2,0) *= -1.0f;

	cv::Mat rot3x3(3,3,CV_32FC1);
	cv::Mat jacob;
  cv::Rodrigues(auxRotVec, rot3x3, jacob);

  float rm[9];
	for(int row = 0; row < 3; ++row) {
		for(int col = 0; col < 3; ++col) {
			rm[col+row*4] = rot3x3.at<float>(row, col);
		}
	}

	modelview_matrix[0] = rm[0];
	modelview_matrix[1] = rm[3];
	modelview_matrix[2] = rm[6];
	modelview_matrix[3] = 0.0f;
	modelview_matrix[4] = rm[1];
	modelview_matrix[5] = rm[4];
	modelview_matrix[6] = rm[7];
	modelview_matrix[7] = 0.0f;
	modelview_matrix[8] = rm[2];
	modelview_matrix[9] = rm[5];
	modelview_matrix[10] = rm[8];
	modelview_matrix[11] = 0.0f;
	modelview_matrix[12] = transVec.at<float>(0,0);
	modelview_matrix[13] = -transVec.at<float>(1,0);
	modelview_matrix[14] = -transVec.at<float>(2,0);
...
void Paper::OgreGetPoseParameters(float position[3], float orientation[4]) throw(cv::Exception){
  ...
  cv::Mat rot3x3(3,3,CV_32FC1);
  cv::Rodrigues(rotVec, rot3x3);

  // calculate axes for quaternion
  float stAxes[3][3];
  // x axis
  stAxes[0][0] = -rot3x3.at<float>(0,0);
  stAxes[0][1] = -rot3x3.at<float>(1,0);
  stAxes[0][2] = +rot3x3.at<float>(2,0);
  // y axis
  stAxes[1][0] = -rot3x3.at<float>(0,1);
  stAxes[1][1] = -rot3x3.at<float>(1,1);
  stAxes[1][2] = +rot3x3.at<float>(2,1);
