...
//  cam.CameraMatrix.copyTo(CameraMatrix);
//  cam.Distorsion.copyTo(Distorsion);
//  CamSize=cam.CamSize;
//  return *this;
//}
...
void CameraModel::saveFile(string filename) const throw(cv::Exception){
  if(!isValid())
...
void CameraModel::loadFile(string filename) throw(cv::Exception){
  ...
  if (cameraMatrix.cols == 0 || cameraMatrix.rows == 0) throw cv::Exception(9007,"File :"+filename+" does not contains valid camera matrix","CameraModel::loadFile",__FILE__,__LINE__);

  if (imageSize.width == -1 || imageSize.height == 0) throw cv::Exception(9007,"File :"+filename+" does not contains valid camera dimensions","CameraModel::loadFile",__FILE__,__LINE__);
 
  if (cameraMatrix.type()!=CV_32FC1) cameraMatrix.convertTo(cameraMatrixF,CV_32FC1);
  else cameraMatrixF = cameraMatrix;

  if (distCoeffs.total() < 5) throw cv::Exception(9007,"File :"+filename+" does not contains valid distortion_coefficients","CameraModel::loadFile",__FILE__,__LINE__);
...
void CameraModel::glGetProjectionMatrix2(float x0,float y0,float width,float height,float znear,float zfar,float proj_matrix[16]){
   
    cv::Mat cameraMatrix = undistortedIntrinsics.getCameraMatrix();
    float fx   = cameraMatrix.at<float>(0,0);
	float skew = cameraMatrix.at<float>(0,1);
    ...
    float qn = -2.0 * (zfar * znear) / depth;

    proj_matrix[0]  =  2*fx/width; 
    proj_matrix[1]  = -2*skew/width;  
    proj_matrix[2]  = (-2*cx+width+2*x0)/width;                   
    proj_matrix[3]  = 0;     
               
    proj_matrix[4]  = 0;
    proj_matrix[5]  = -2*fy/height;
    proj_matrix[6]  = (-2*cy+height+2*y0)/height;
    proj_matrix[7]  = 0;
...
void CameraModel::updateUndistortion() {
  cv::Mat undistortedCameraMatrix = cv::getOptimalNewCameraMatrix(distortedIntrinsics.getCameraMatrix(), distCoeffs, distortedIntrinsics.getImageSize(), false);
   cv::initUndistortRectifyMap(distortedIntrinsics.getCameraMatrix(),distCoeffs, cv::Mat(), undistortedCameraMatrix, distortedIntrinsics.getImageSize(), CV_16SC2, __undistortMapX, __undistortMapY);
