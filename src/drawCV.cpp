...
void drawCV::projectPaper(cv::Mat& image, Paper& paper, CameraProjectorSystem& cameraProjector, cv::Mat& video,  cv::Mat& out){
  ...
  cout << "M: " << M << endl;
  cv::warpPerspective ( video, out,  M, Size(640,480),cv::INTER_NEAREST );
...
void drawCV::projectPaper0(cv::Mat& image, Paper& paper, CameraProjectorSystem& cameraProjector, int index){
  ...
  if (index == 2){
    putText(image, providers[index], cv::Point(centroid.x-105,centroid.y-25) ,FONT_HERSHEY_SIMPLEX, 0.5, CV_RGB(255,255,255),2);
    putText(image, "[AYUDA]", cv::Point(centroid.x-80,centroid.y) ,FONT_HERSHEY_SIMPLEX, 0.5, CV_RGB(255,255,255),2);
    putText(image, invoices[index],  cv::Point(centroid.x-105,centroid.y+25) ,FONT_HERSHEY_SIMPLEX, 0.5, CV_RGB(255,255,255),2);
  }
  else{
    putText(image, providers[index], cv::Point(centroid.x-105,centroid.y-25) ,FONT_HERSHEY_SIMPLEX, 0.5,CV_RGB(255,255,255),2);
    putText(image, invoices[index],  cv::Point(centroid.x-105,centroid.y) ,FONT_HERSHEY_SIMPLEX, 0.5, CV_RGB(255,255,255),2);
  }
  // for(size_t i = 0; i < imagePoints.size(); i++)
  //	  cv::circle(image, imagePoints[i], 10,  CV_RGB(255,255,255), -1, 8);
